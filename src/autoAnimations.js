    ...
    if (game.modules.get("midi-qol")?.active) {
        ...
        switch (game.system.id) {
            ...
            case 'ose':
                ...
                break;
			case "tagmar_rpg":
				Hooks.on("createChatMessage", async (msg) => { setupTagmar(msg) });
				break;
			case "tagmar":
				Hooks.on("createChatMessage", async (msg) => { setupTagmar(msg) });
				break;
...
/*
/ Sets Handler for Tagmar RPG
*/
async function setupTagmar(msg) {
	if (killAllAnimations) { return; }
    let handler = await flagHandler.make(msg);
	if (typeof(handler) === "undefined" || !handler.item || !handler.actorToken || handler.animKill) {
        return;
    }
	if (game.user.id === msg.user.id) {
        switch (true) {
